...
(() => {
  ...
  window.addEventListener('iris-data-sync', (e) => {
    console.log('[Dashboard] Data sync received: ' + e.detail.type);
    render(); 
  });
  ...
  // ── Main Render Logic ──
  function render() {
    console.log('[Dashboard] Initializing premium render sequence...');
    ...
    // 2. User Info (Topbar/Sidebar)
    const adminProfile = isAdmin ? (Storage.getAdminProfile ? Storage.getAdminProfile(session.userId) : null) : null;
    const currentName = (isAdmin ? adminProfile?.name : profile?.name) || session.displayName;
  ...
  function renderStats(projects, allProfiles, profile) {
    ...
    if (isAdmin) {
      ...
      const teamSize = allProfiles.filter(p => p.internship?.company === (profile?.internship?.company || 'N/A')).length;
    ...
    recentProjList.innerHTML = recent.map((p, i) => `
      <div class="proj-item anim-stagger visible card-3d" style="transition-delay: ${i * 0.15}s">
        ...
        <div class="proj-thumb" style="background:${p.screenshot ? 'none' : '#8b5cf6'}">
          ${p.screenshot ? `<img src="${p.screenshot}" alt="${p.title}">` : p.title[0]}
        </div>
        <div class="proj-info">
  ...
  function renderRecentReports(allProfiles) {
    ...
    const recent = reports.sort((a,b) => (b.timestamp || 0) - (a.timestamp || 0)).slice(0, 5);
    list.innerHTML = recent.map((r, i) => {
      const p = allProfiles.find(prof => prof.userId === r.userId) || { name: 'Unknown' };
      return `
        <div class="proj-item visible card-3d" style="padding: 8px 12px; transition-delay: ${i * 0.05}s">
          <div class="glare" aria-hidden="true"></div>
