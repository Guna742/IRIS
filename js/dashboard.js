...
(() => {
  ...
  function render() {
    try {
      ...
      if (welcomeTitle) {
        if (isAdmin) {
          welcomeTitle.innerHTML = `
            <div class="dash-hero-title">Welcome, Command Center <span class="material-symbols-outlined" style="font-size: 24px; vertical-align: middle; color: var(--clr-accent);">settings_account_box</span></div>
            <div class="dash-hero-sub">Managing ${allProfiles.length} active interns today.</div>
          `;
        } else {
          const name = (currentName || 'Intern').split(' ')[0];
          welcomeTitle.innerHTML = `
            <div class="dash-hero-title ripple-text">Hey ${name}, You're in the Top 13% <span class="material-symbols-outlined" style="font-size: 24px; vertical-align: middle; color: var(--clr-success);">rocket_launch</span></div>
      ...
      if (xpBar && !isAdmin && profile) {
        ...
        // Update badge icon if it exists
        const badge = document.getElementById('rank-percent-badge');
        if (badge) {
            badge.innerHTML = `Top 13% today <span class="material-symbols-outlined" style="font-size: 14px; vertical-align: middle;">rocket_launch</span>`;
        }
      }
    ...
    if (isAdmin) {
      ...
      el.innerHTML = `
        <div style="display:flex; align-items:center; gap:20px;">
          <div class="insights-icon-glow"><span class="material-symbols-outlined" style="font-size: 32px; color: var(--clr-accent);">psychology</span></div>
          <div style="flex:1">
            <h4 style="font-size:14px; margin-bottom:4px; font-family:var(--font-comic)">System Intelligence <span class="material-symbols-outlined" style="font-size: 16px; vertical-align: middle;">psychology</span></h4>
            <div class="insights-text">
                ${activeCount > 0 
                  ? `<span style="color:var(--clr-success); font-weight:700;">${activeCount} interns active</span> in the last 48 hours. <span class="material-symbols-outlined" style="font-size: 14px; vertical-align: middle;">rocket_launch</span>` 
                  : `<span style="color:var(--clr-text-muted);">No collective traces detected...</span> standing by for intern activity. <span class="material-symbols-outlined" style="font-size: 14px; vertical-align: middle;">satellite_alt</span>`}
                ${projects.length > 0 
                  ? `<br>The team improved performance by <b>12%</b> this week. Unbelievable pace! <span class="material-symbols-outlined" style="font-size: 14px; vertical-align: middle; color: #ff5722;">local_fire_department</span>` 
      ...
      if (reports.length > 0) {
          ...
          scoringMsg = `
            <div style="margin-top:10px; padding:10px; background:rgba(255,255,255,0.03); border-radius:10px; border-left:4px solid ${evalRes.score >= 70 ? '#10B981' : '#F59E0B'}">
                ...
                <div style="font-size:11px; opacity:0.8; line-height:1.4"><span class="material-symbols-outlined" style="font-size: 14px; vertical-align: middle; color: var(--clr-accent);">psychology</span>: ${evalRes.feedback}</div>
            </div>`;
      }

      const rankPercent = 94;
      let insightMsg = reportsToday === 0 
          ? `You missed today's report! Start your mission to preserve your streak <span class="material-symbols-outlined" style="font-size: 14px; vertical-align: middle; color: #ff9800;">warning</span>` 
          : `You're close to Top 5! Keep this momentum to reach <b>Pro</b> status. <span class="material-symbols-outlined" style="font-size: 14px; vertical-align: middle; color: #ffeb3b;">bolt</span>`;

      el.innerHTML = `
        <div style="display:flex; align-items:center; gap:20px;">
          <div class="insights-icon-glow"><span class="material-symbols-outlined" style="font-size: 32px; color: var(--clr-accent);">psychology</span></div>
          <div style="flex:1">
            <h4 style="font-size:14px; margin-bottom:4px; font-family:var(--font-comic)">AI Journey Guide <span class="material-symbols-outlined" style="font-size: 16px; vertical-align: middle;">psychology</span></h4>
            <div class="insights-text">
                ${points > 0 ? `You've outperformed <b>${rankPercent}% of interns</b> this week! <span class="material-symbols-outlined" style="font-size: 14px; vertical-align: middle; color: #ffea00;">stars</span>` : 'Let\'s start your journey!'} 
  ...
  function renderJourney(isAdmin, profile, projects) {
    ...
    if (isAdmin) {
        ...
        if (cardTitle) cardTitle.innerHTML = 'Platform Growth Matrix <span class="material-symbols-outlined" style="font-size: 18px; vertical-align: middle;">monitoring</span>';
    ...
    const steps = [
        { title: 'The Start <span class="material-symbols-outlined" style="font-size: 14px; vertical-align: middle;">child_care</span>', desc: 'Welcome to IRIS', date: 'Day 1', active: true },
        { title: 'First Drop <span class="material-symbols-outlined" style="font-size: 14px; vertical-align: middle;">rocket_launch</span>', desc: 'Log your initial task', date: 'Done', active: points > 0 },
        { title: 'Consistency King <span class="material-symbols-outlined" style="font-size: 14px; vertical-align: middle;">workspace_premium</span>', desc: 'Maintain 5-day streak', date: profile?.streak >= 5 ? 'Active' : `${profile?.streak || 0}/5`, active: profile?.streak >= 5 },
        { title: 'Pro Transition <span class="material-symbols-outlined" style="font-size: 14px; vertical-align: middle;">bolt</span>', desc: 'Reach 1000 XP', date: points >= 1000 ? 'Unlocked' : `${points}/1000`, active: points >= 1000 },
    ];
  ...
  function renderSkills(isAdmin, profile) {
    ...
    if (isAdmin) {
        const cardTitle = el.closest('.dash-card')?.querySelector('.dash-card-title');
        if (cardTitle) cardTitle.innerHTML = 'Admin Oversight <span class="material-symbols-outlined" style="font-size: 18px; vertical-align: middle;">verified_user</span>';
